(function($, window) {
	...
	var template = '<div id="{{id}}" class="mdui-slider mdui-preview-images mdui-fullscreen"><div class="mdui-preview-header">{{header}}</div><div class="mdui-slider-group"></div><div class="mdui-preview-footer mdui-hidden">{{footer}}</div><div class="mdui-preview-loading"><span class="mdui-spinner mdui-spinner-white"></span></div></div>';
	var itemTemplate = '<div class="mdui-slider-item mdui-zoom-wrapper {{className}}"><div class="mdui-zoom-scroller"><img src="{{src}}" data-preview-lazyload="{{lazyload}}" style="{{style}}" class="mdui-zoom"></div></div>';
	var defaultGroupName = '__DEFAULT';
	var div = document.createElement('div');
	var imgId = 0;
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			id: '__mdui_PREVIEWIMAGE',
			zoom: true,
			header: '<span class="mdui-preview-indicator"></span>',
			footer: ''
		}, options || {});
		this.init();
		this.initEvent();
	...
	proto.initEvent = function() {
		...
		this.element.addEventListener('slide', function(e) {
			if (self.options.zoom) {
				var lastZoomerEl = self.element.querySelector('.mdui-zoom-wrapper:nth-child(' + (self.lastIndex + 1) + ')');
				if (lastZoomerEl) {
	...
	proto.addImage = function(img) {
		...
		if (img.__mdui_img_data && img.__mdui_img_data.src === src) { //已缓存且图片未变化
			this.groups[group].push(img.__mdui_img_data);
		} else {
	...
	proto.open = function(index, group) {
		...
		if (typeof index === "number") {
			...
			group = group || defaultGroupName;
			this.addImages(group, index); //刷新当前group
			this.openByGroup(this.groups[group].indexOf(index.__mdui_img_data), group);
		}
	};
